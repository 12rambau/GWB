gwb = {
    'acc':  {'command': 'GWB_ACC', 'folder': 'acc',  'offset': 23},
    'dist': {'command': 'GWB_DIST', 'folder': 'dist', 'offset': 17},
    'fad':  {'command': 'GWB_FAD', 'folder': 'fad',  'offset': 28},
    'frag': {'command': 'GWB_FRAG', 'folder': 'frag', 'offset': 32},
    'lm':   {'command': 'GWB_LM', 'folder': 'lm',   'offset': 14},
    'mspa': {'command': 'GWB_MSPA', 'folder': 'mspa', 'offset': 23},
    'p222':   {'command': 'GWB_P222', 'folder': 'p2', 'offset': 23},
    'parc': {'command': 'GWB_PARC', 'folder': 'parc', 'offset': 17},
    'rss':  {'command': 'GWB_RSS', 'folder': 'rss',  'offset': 14},
    'spa':  {'command': 'GWB_SPA', 'folder': 'spa',  'offset': 18},
    'rec':  {'command': 'GWB_REC', 'folder': 'rec',  'offset': 20}
}