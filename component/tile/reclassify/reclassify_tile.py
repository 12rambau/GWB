...
class CustomizeTile(v.Card):
    ...
    def get_classes_files(self):
        ...
        self.classes_files = [
            str(f) for f in (list(look_up_folder) + list(module_classes_folder))
        ]
...
class ReclassifyUI(v.Card, sw.SepalWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.w_class_file = v.Select(
            label="Select a classes file", v_model="", dense=True
        )
    ...
    def reclassify_and_save(self, *args):
        ...
        map_values = {
            k: v["value"] if "text" in v else v for k, v in change_matrix.items()
        }
        ...
        reclassify_from_map(
            in_raster, map_values, dst_raster=dst_raster, overwrite=True
        )
