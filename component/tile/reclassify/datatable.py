...
class ClassTable(v.DataTable, sw.SepalWidget):
    ...
    def _edit_event(self, widget, event, data):
        ...
        dial = EditDialog(
            schema=self.schema, default=self.v_model[0], table=self, transition=False
        )
        with self.dialog:
            display(dial)
    ...
    def _add_event(self, widget, event, data):
        ...
        dial = EditDialog(schema=self.schema, table=self)
        with self.dialog:
            display(dial)
...
class EditDialog(v.Dialog):
    ...
    def __init__(self, table, schema, *args, default=None, **kwargs):
        ...
        self.children = [
            v.Card(
                class_="pa-4",
                children=[v.CardTitle(children=[self.title])]
                + self._get_widgets()
                + [actions],
            )
        ]
    ...
    def _get_widgets(self):
        ...
        for title, type_ in self.schema.items():
            ...
            if self.default:
                ...
                if title == "id":
                    widget.v_model = self._get_index()
...
class SaveDialog(v.Dialog):
    ...
    def __init__(self, table, out_path, *args, **kwargs):
        ...
        self.w_file_name = v.TextField(
            label="Insert output file name", type="string", v_model="new_table.csv"
        )
        ...
        info = (
            sw.Alert()
            .add_msg("The table will be stored in {}".format(str(out_path)))
            .show()
        )
